...
**Q: Why regress LD1 on PCs? Aren't they derived from same data?**
A: Yes, but explores relationship: "Can unsupervised PCs predict supervised LD1?" Tests if gender info captured by variance.

**Q: What does high R² mean practically?**
A: Gender information (LD1) is strongly represented in principal components. PCA captures gender-related features.

**Q: Why use Welch's instead of pooled t-interval?**
A: Welch's doesn't assume equal variances. More robust. Slightly wider CI if variances differ.

**Q: What if residual plot shows pattern?**
A: Consider non-linear terms, interactions, or different model family (e.g., regularized regression).

**Q: How to interpret coefficient for PC5 = 0.23, p = 0.08?**
A: 1 unit increase in PC5 predicts 0.23 unit increase in LD1. Not quite significant at α=0.05 (borderline).

**Q: Why 95% confidence level?**
A: Convention in statistics. Balance between precision and confidence. Can use 90% or 99% if needed.
